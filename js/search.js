...
const SEARCH_CONFIG = {
  ...
  STORAGE_KEY: TICS.CONFIG.STORAGE_KEYS.SEARCH_HISTORY
};
...
function searchProducts(query) {
  ...
  const products = window.ProductsManager ? 
    (window.productsData || []) : [];
...
function createSuggestionHTML(suggestion) {
  ...
  if (suggestion.type === 'product') {
    additionalInfo = `
      <div class="suggestion-details">
        <img src="${suggestion.image}" alt="${TICS.sanitizeHTML(suggestion.text)}" loading="lazy">
        <span class="suggestion-category">${TICS.sanitizeHTML(suggestion.category)}</span>
      </div>
    `;
  }
  ...
  return `
    <div class="search-suggestion" 
         ...
         data-text="${TICS.sanitizeHTML(suggestion.text)}"
...
function highlightSearchTerm(text, searchTerm) {
  if (!searchTerm || searchTerm.length < SEARCH_CONFIG.MIN_SEARCH_LENGTH) {
    return TICS.sanitizeHTML(text);
  }
  
  const regex = new RegExp(`(${searchTerm.replace(/[.*+?^${}()|[\\]\\\\]/g, '\\$&')})`, 'gi');
  return TICS.sanitizeHTML(text).replace(regex, '<strong>$1</strong>');
}
...
function performSearch(query) {
  ...
  if (!searchQuery || searchQuery.length < SEARCH_CONFIG.MIN_SEARCH_LENGTH) {
    TICS.ToastManager.warning(`Ingresa al menos ${SEARCH_CONFIG.MIN_SEARCH_LENGTH} caracteres para buscar`);
    return;
  }
...
function addToSearchHistory(searchTerm) {
  ...
  TICS.StorageManager.save(SEARCH_CONFIG.STORAGE_KEY, searchHistory);
}
...
function loadSearchHistory() {
  searchHistory = TICS.StorageManager.load(SEARCH_CONFIG.STORAGE_KEY, []);
}
 ...
 */
const handleSearchInput = TICS.debounce((e) => {
...
function initShopPageSearch() {
  ...
  const searchQuery = TICS.getQueryParam('search');
...
function showMobileSearchOverlay() {
  ...
  // Handle input
  mobileInput.addEventListener('input', TICS.debounce((e) => {
